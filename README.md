...
| [`whatsapp-audio-transcript`](./whatsapp-audio-transcript/SKILL.md) | Trascrive **vocali WhatsApp** (`.opus`/`.ogg`, singoli o a raffica) con Whisper on-device: risponde in chat con il testo pulito, niente documento pesante. Gestisce più vocali consecutivi come un unico messaggio. Per meeting/podcast/interviste usa `audio-video-transcript`. |
...
| [`whatsapp-audio-transcript`](./whatsapp-audio-transcript/SKILL.md) | Transcribes **WhatsApp voice notes** (`.opus`/`.ogg`, single or in a burst) with on-device Whisper: replies in chat with the clean text, no heavy document. Handles several consecutive notes as one message. Use `audio-video-transcript` instead for meetings/podcasts/interviews. |
